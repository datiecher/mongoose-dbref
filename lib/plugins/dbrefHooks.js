...
var dbrefHooks = function dbrefHooks (schema, options) {
    ...
    schema.static('fetch', function (dbref, callback, strict) {
        ...
        // Dereference is deprecated in new Mongo versions so we implement it in
        // userland if it's needed
        if (this.db.db.dereference) {
            this.db.db.dereference(dbref, callback);
        } else {
            var collection = this.db.db.collection(dbref.namespace);
            collection.findOne({ _id: dbref.oid }, callback);
        }
    });
};
